...
public class Day4 {
    static String input = "sayndz zfxlkl attjtww cti sokkmty brx fhh suelqbp\n" +
    ...
    static int prob1(String input) {
        int sum = 0;
        for (String line : input.split("\\r?\\n")) {
            sum += prob1Valid(line) ? 1 : 0;
        }
        return sum;
    }

    static int prob2(String input) {
        int sum = 0;
        for (String line : input.split("\\r?\\n")) {
            sum += prob2Valid(line) ? 1 : 0;
        }
        return sum;
    }


    static boolean prob1Valid(String input) {
    ...
    static boolean prob2Valid(String input) {
        String[] words = input.split(" ");
        Set<String> exists = new HashSet<>();
